...
function logActivity($action, $details = '') {
    global $pdo; 
    if (!isset($pdo)) { // Comprobar si $pdo está seteado
        error_log("logActivity Error: \$pdo no está disponible.");
        return;
    }
    try {
        $query = "INSERT INTO admin_logs (admin_user_id, action, details, ip_address, user_agent) VALUES (:admin_user_id, :action, :details, :ip_address, :user_agent)";
        $stmt = $pdo->prepare($query);
        $admin_id = $_SESSION['admin_id'] ?? null; // Asumiendo que guardas admin_id en sesión
        $ip_address = $_SERVER['REMOTE_ADDR'] ?? null;
        $user_agent = $_SERVER['HTTP_USER_AGENT'] ?? null;

        $stmt->bindParam(':admin_user_id', $admin_id, PDO::PARAM_INT);
...
function getStoreStatus() {
    ...
    if (!isset($pdo)) {
        ...
        return ['is_open' => false, 'last_updated' => date('Y-m-d H:i:s')]; 
    }
    ...
    try {
        ...
        if ($row) {
            $isOpen = false;
            if (!$row['is_closed']) {
                // Tienda no marcada como cerrada para el día, verificar horarios
                $currentTime = time();
                ...
                if ($openTime !== false && $closeTime !== false) {
                    if ($openTime <= $currentTime && $currentTime <= $closeTime) {
                        $isOpen = true;
                    } else if ($openTime > $closeTime) { // Caso de cierre después de medianoche (ej. abre 22:00, cierra 02:00)
            ...
            return [
                'is_open' => $isOpen,
                'details' => $row // Devuelve todos los detalles para posible uso en UI
            ];
        }
    } catch (PDOException $e) {
        error_log("Error en getStoreStatus: " . $e->getMessage());
    }
    return ['is_open' => false, 'details' => null];
}
